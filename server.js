...
const db = mysql.createConnection({
    host: "bahwaimrgtkuvsofzosn-mysql.services.clever-cloud.com",
    user: "ufebupvobikajmfp",
    password: "msIDPDNjsVfJRkvBwe77",
    database: "bahwaimrgtkuvsofzosn"
})
...
app.put("/admin/toggle-student-encoding", (req, res) => {
  const { studentUserId, enabled } = req.body;
  const encodingValue = enabled ? 'on' : 'off';

  // console.log("Toggle student encoding request:", { studentUserId, enabled, encodingValue });
  ...
  const query = `UPDATE students SET encodeGrade = ? WHERE studentUser_id = ?`;

  db.query(query, [encodingValue, studentUserId], (err, result) => {
    if (err) {
    ...
    res.json({ 
      ...
      encodeGrade: encodingValue
    });
  });
});

app.put("/admin/toggle-all-student-encoding", (req, res) => {
  const { studentUserIds, enabled } = req.body;
  const encodingValue = enabled ? 'on' : 'off';

  // console.log("Toggle all students encoding request:", { studentUserIds, enabled, encodingValue });
  ...
  const query = `UPDATE students SET encodeGrade = ? WHERE studentUser_id IN (${placeholders})`;
  const params = [encodingValue, ...studentUserIds];
  ...
  db.query(query, params, (err, result) => {
    if (err) {
      ...
      return res.status(500).json({ 
        success: false, 
        message: "Failed to update encoding for students",
        error: err.message 
      });
    }
    ...
    res.json({ 
      success: true, 
      message: `Encoding ${enabled ? 'enabled' : 'disabled'} for ${result.affectedRows} students`,
      affectedRows: result.affectedRows,
      encodeGrade: encodingValue
    });
  });
...
// Replace the existing endpoint in server.js (around line 1050)
// This ensures student_id (A22-0001 format) is included in the response
...
app.get("/admin/class-students/:teacher_id/:subject_id/:section_code", (req, res) => {
  ...
  const query = `
    ...
    LEFT JOIN grades g ON s.studentUser_id = g.studentUser_id 
      AND g.subject_id = ss.subject_id 
      AND g.teacher_id = ss.teacher_id
    LEFT JOIN handle_subject hs ON ss.subject_id = hs.subject_id 
      AND ss.teacher_id = hs.teacher_id 
      AND ss.sectionCode = hs.sectionCode
    WHERE ss.teacher_id = ? 
      AND ss.subject_id = ? 
...
// Add this updated endpoint to your server.js file
// Replace the existing /teacher/classes/:teacher_id endpoint
...
app.get("/teacher/classes/:teacher_id", (req, res) => {
  ...
  const query = `
    ...
    LEFT JOIN student_subject ss ON ss.subject_id = hs.subject_id 
      ...
      AND ss.sectionCode = hs.sectionCode
    WHERE hs.teacher_id = ?
    GROUP BY hs.subject_id, hs.sectionCode, s.subject_code, s.subject_name, 
             d.department_name, hs.schoolyear, hs.semester
    ORDER BY hs.schoolyear DESC, hs.semester, s.subject_name, hs.sectionCode
  `;
...
// Add this updated endpoint to your server.js file
// Replace the existing /teacher/section-grades endpoint
    ...
    db.query(teacherQuery, (err, teacherResults) => {
      ...
      db.query(adminQuery, (err, adminResults) => {
        if (err) {
          ...
          return res.status(500).json({ 
            success: false, 
            message: "Failed to fetch admin passwords",
            error: err.message 
          });
        }

        const allUsers = [...studentResults, ...teacherResults, ...adminResults];
        res.json(allUsers);
      });
    });
  });
...
app.put("/api/change-password-admin", async (req, res) => {
  ...
  db.query(verifyQuery, [username], async (err, results) => {
    if (err) {
      return res.status(500).json({ 
        success: false, 
        message: "Database error" 
    ...
    db.query(updateQuery, [hashedNewPassword, newPassword, account.admin_id], (err, result) => {
      if (err) {
        return res.status(500).json({ 
          success: false, 
          message: "Failed to update password" 
...
app.post("/admin/update-grades-bulk", (req, res) => {
  const { teacher_id, subject_id, grades } = req.body;

  if (!teacher_id || !subject_id || !grades || !Array.isArray(grades) || grades.length === 0) {
    return res.status(400).json({ 
      success: false, 
  ...
  grades.forEach(gradeData => {
    const promise = new Promise((resolve) => {
      const { studentUser_id, midterm_grade, final_grade } = gradeData;
      ...
      db.query(checkQuery, [studentUser_id, subject_id, teacher_id], (err, results) => {
        if (err) {
        ...
        if (results.length > 0) {
          ...
          db.query(updateQuery, [midterm_grade, final_grade, results[0].grade_id], (err) => {
            if (err) {
          ...
          const insertQuery = `
            INSERT INTO grades (studentUser_id, subject_id, teacher_id, midterm_grade, final_grade, academic_year, semester)
            VALUES (?, ?, ?, ?, ?, '2024-2025', '1st')
          `;

          db.query(insertQuery, [studentUser_id, subject_id, teacher_id, midterm_grade, final_grade], (err) => {
            if (err) {
  ...
  Promise.all(promises).then(() => {
    res.json({ 
      ...
      message: errorCount === 0 
        ...
        : `Updated ${updatedCount} grade(s) with ${errorCount} error(s)`
    });
  });
});
