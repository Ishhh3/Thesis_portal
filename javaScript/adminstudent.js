...
function loadAllStudents() {
    ...
    const link = "https://mseufportal.onrender.com/admin/students";
...
function displayStudents(students) {
    ...
    students.forEach((student) => {
        ...
        row.setAttribute("data-user-id", student.studentUser_id);
        ...
        row.setAttribute("data-encode", student.encodeGrade || "off");

        const encodeStatus = student.encodeGrade === 'on';
        const toggleBtn = document.createElement("button");
        toggleBtn.className = `toggle-btn ${encodeStatus ? 'on' : 'off'}`;
        toggleBtn.textContent = encodeStatus ? 'ON' : 'OFF';
        toggleBtn.onclick = function(e) { 
            e.stopPropagation(); 
            toggleStudentEncoding(student.studentUser_id, encodeStatus, toggleBtn);
        };
        ...
        row.innerHTML = `
            ...
            <td class="encode-cell"></td>
        `;

        row.querySelector('.encode-cell').appendChild(toggleBtn);
        
        row.querySelector('td:not(.encode-cell)').onclick = function() { viewStudentInfo(row); };
        row.querySelector('td:nth-child(2)').onclick = function() { viewStudentInfo(row); };
        row.querySelector('td:nth-child(3)').onclick = function() { viewStudentInfo(row); };
        row.querySelector('td:nth-child(4)').onclick = function() { viewStudentInfo(row); };

        tbody.appendChild(row);
    });
}

function toggleStudentEncoding(studentUserId, currentStatus, button) {
    const newStatus = !currentStatus;
    
    console.log("Toggling encoding:", { studentUserId, currentStatus, newStatus });
    
    fetch('https://mseufportal.onrender.com/admin/toggle-student-encoding', {
        method: 'PUT',
        headers: {
    ...
    .then(data => {
        ...
        if (data.success) {
            button.className = `toggle-btn ${newStatus ? 'on' : 'off'}`;
            button.textContent = newStatus ? 'ON' : 'OFF';
            
            const studentIndex = allStudents.findIndex(s => s.studentUser_id === studentUserId);
            if (studentIndex !== -1) {
                allStudents[studentIndex].encodeGrade = newStatus ? 'on' : 'off';
            }
            
            alert(`Encoding ${newStatus ? 'enabled' : 'disabled'} successfully!`);
        } else {
            alert('Failed to update encoding status: ' + (data.message || 'Unknown error'));
        }
    })
    .catch(error => {
        console.error('Error:', error);
        alert('Error updating encoding status: ' + error.message);
    });
}

function toggleAllStudentEncoding(enable) {
    const studentUserIds = allStudents.map(s => s.studentUser_id);
    
    console.log("Toggling all students:", { studentUserIds, enable });
    
    fetch('https://mseufportal.onrender.com/admin/toggle-all-student-encoding', {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
        },
    ...
    .then(data => {
        console.log("Response data:", data);
        if (data.success) {
            alert(`Encoding ${enable ? 'enabled' : 'disabled'} for all students!`);
            loadAllStudents();
        } else {
            alert('Failed to update encoding status: ' + (data.message || 'Unknown error'));
        }
...
function fetchStudentGrades(studentId) {
    const link = `https://mseufportal.onrender.com/student/grades/${studentId}`;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const enableAllBtn = document.getElementById('enableAllEncoding');
    const disableAllBtn = document.getElementById('disableAllEncoding');
    
    if (enableAllBtn) {
        enableAllBtn.addEventListener('click', () => toggleAllStudentEncoding(true));
    }
    if (disableAllBtn) {
        disableAllBtn.addEventListener('click', () => toggleAllStudentEncoding(false));
    }
});