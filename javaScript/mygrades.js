...
function loadGrades() {
    ...
    const url = `https://mseufportal.onrender.com/student/grades/${loggedInUser.student_id}`;
...
function displayGrades(grades) {
    ...
    tableBody.innerHTML = grades.map(grade => {
        ...
        if (finalGrade >= 1.00 && finalGrade <= 3.00) {
            ...
            remark = 'Failed';
            remarkClass = 'grade-failed';
        } else if (finalGrade === 0.00) {
            remark = 'Dropped';
            remarkClass = 'grade-dropped';
        }

        const midterm = grade.midterm_grade !== null ? grade.midterm_grade : 0;
        // Display "INC" if final grade is 6.00, otherwise show the grade
        let finals;
        if (grade.final_grade !== null && parseFloat(grade.final_grade) === 6.00) {
            finals = 'INC';
        } else if (grade.final_grade !== null) {
            finals = parseFloat(grade.final_grade).toFixed(2);
        } else {
            finals = '0.00';
        }
