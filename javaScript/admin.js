...
async function loadDashboardData() {
    try {
        const adminResponse = await fetch(`https://mseufportal.onrender.com/admin/dashboard/${loggedInUser.admin_id}`, {
            mode: 'cors'
        });
        ...
        const statsResponse = await fetch('https://mseufportal.onrender.com/admin/stats', {
            mode: 'cors'
        });
...
async function showTeacherClasses(teacherId, teacherName) {
    ...
    try {
        const response = await fetch(`https://mseufportal.onrender.com/admin/teacher-classes/${teacherId}`, {
            mode: 'cors'
        });
...
async function viewClassStudents(teacherId, subjectId, subjectName) {
    ...
    try {
        const response = await fetch(
            `https://mseufportal.onrender.com/admin/teacher-sections/${teacherId}/${subjectId}`,
            { mode: 'cors' }
        );
...
async function handleSectionChange(event) {
    ...
    try {
        const response = await fetch(
            `https://mseufportal.onrender.com/admin/class-students/${currentTeacherId}/${currentSubjectId}/${sectionCode}`,
            { mode: 'cors' }
        );
...
async function toggleEncoding(button, teacherId, encodingType) {
    ...
    try {
        const response = await fetch('https://mseufportal.onrender.com/admin/toggle-encoding', {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
